...
Now, let's add the `presto` service to the **Integration Platform**. 
...
In order to connect from Presto to data in an Object Store (MinIO) we also need a Hive Metastore running. Also add the following two services to the `docker-compose.override.yml`, which start the metastore service as well as another PostgreSQL instance holding the metadata schema. 
...
Now restart the stack using `docker-compose up -d` and 3 new services should get added to the running stack. 
...
connection-url=jdbc:postgresql://postgresql:5432/orderproc
connection-user=orderproc
connection-password=orderproc
```
...
In a browser window, navigate to <http://integrationplatform:9000> and you should see login screen. Enter `V42FCGRVMK24JJ8DHUYG` into the **Access Key** and  `bKhWxVF3kQoLY9kFmt91l+tDrEoZjqnWXzY9Eza ` into the **Secret Key** field and click on the connect button.  
...
In order to use the `s3cmd` from within Apache Zeppelin, we need to provide the `s3cfg` config file, which configures the object store to use as default. The [../01-environment/docker/conf/s3cfg](../01-environment/docker/conf/s3cfg) holds this configuration of the Minio instance and is part of the base environment. 

This config file is added via a volume mapping to the `zeppelin` service in the `docker-compose.yml` file, as shown in the snippet below
...
In a browser window, navigate to <http://integrationplatform:38081> and you should see the Apache Zeppelin homepage. Create a new notebook using the **Create new note** link and then  use the `s3cmd` command from within a cell in a notebook using the `%sh` interpreter.
...
Navigate to the MinIO UI (<http://integrationplatform:9000>) and you should see the newly created bucket in the left. 
...
A Postgresql instance called `postgresql` is available as part of the **Integration platform** described [here](../01-environment).


There is also a table `driver` preloaded inside the schema `truck`. This has been done through the file [`create-driver.sql`](../01-environment/docker/sql/create-driver.sql), which is copied into the `docker-entrypoint-initdb.d` folder. The content of this folder is automatically executed upon starting the container. The config snippet below shows the important part of the service definition in the `docker-compose.yml`:
...
You can use the `Adminer` service to browse the data. Navigate to <http://integrationplatform:28081> and log in to the `Postgresql` database using the `orderproc` database. 
...
Presto also provides the [Presto UI](http://integrationplatform:8089) for monitoring the queries executed on the presto server. 
